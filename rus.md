...
интерфейсами, если могу их достать. Но я не думаю, что это произойдет с
Grunt<sup>1</sup>.
...
названием *Gruntfile.js*<sup>2</sup>.
...
Если Вы из тех, кто лучше обучается, смотря видеоуроки, то я сделал для Вас
скринкаст, дополняющий эту статью. Он опубликован на CSS-Tricks:
[First Moments with Grunt][screencast]
...
<sup>1</sup> Может быть, когда-нибудь кто-нибудь сделать красивое приложение
Grunt для Вашей операционной системы. Но я не уверен, что этот день наступит.
Конфигурация плагинов — это важная часть использования Grunt. Каждый плагин
немного отличается от остальных, в зависимости от того, что он делает. Это
означает уникальный интерфейс для каждого отдельного плагина, а это рискованное
занятие.

Возможно, неплохим компромиссом станет аддон [Grunt DevTools][34] для Chrome.

<sup>2</sup> *Gruntfile.js* в документации и примерах часто называется
*Gruntfile*. Не называйте этот файл буквально, *Gruntfile*, это не заработает.
